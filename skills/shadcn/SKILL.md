...
Run `npx shadcn@latest info --json` when this skill is invoked and `components.json` exists. Read its output for the current project context. If `components.json` is absent, the project has not adopted shadcn/ui; use this skill only as reference until the user explicitly requests that setup. The skill itself does not authorize installing components or changing the project's UI architecture.
...
1. **Get project context** — run `npx shadcn@latest info --json` when `components.json` exists. Refresh it after configuration changes.
